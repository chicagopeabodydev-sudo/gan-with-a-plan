...
NOTE: this example uses OpenAI to answer questions which is not always the endpoint for questions entered into a chat interface -- just focus on the Streamlit code to accept questions and return LLM results
...
## Example 7 - Chatbot UI with Claude (Anthropic SDK)

Same chat UI as Example 6, but wired to `claude-sonnet-4-6` via the Anthropic SDK. Requires `ANTHROPIC_API_KEY` set in the environment.

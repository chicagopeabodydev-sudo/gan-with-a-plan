# Adversarial Process Skill — Python Examples: Planner
...
- `persist_session` is not set — defaults to `False`. The Planner is stateless.
- Planner writes `spec.md` to disk; the returned text is a fallback if the file isn't written.
- Tool list is intentionally narrow: `Read` and `Write` only. No shell access.
